...
namespace PeteTimesSix.SelfControl.Extensions
{
    public static class Listing_StandardExtensions
    {
        ...
        public static void CheckboxLabeled(this Listing_Standard instance, string label, ref bool checkOn, string tooltip = null, Action onChange = null, Texture2D texOn = null, Texture2D texOff = null, bool disabled = false)
        {
            ...
            if (instance.BoundingRectCached == null || rect.Overlaps(instance.BoundingRectCached.Value))
            {
                ...
                Widgets_Custom.CheckboxLabeledTextured(rect, label, ref checkOn, disabled, texOn, texOff, false); 
                if (!disabled && checkOn != valueBefore)
                {
                    onChange?.Invoke();
                }
            }
        ...
        public static void SliderLabeled(this Listing_Standard instance, string label, ref float value, float min, float max, float displayMult = 1, int decimalPlaces = 0, string valueSuffix = "", string tooltip = null, Action onChange = null, bool disabled = false)
        {
            instance.Label($"{label}: {(value * displayMult).ToString($"F{decimalPlaces}")}{valueSuffix}", tooltip: tooltip);
            var valueBefore = value; 
            float num = Widgets.HorizontalSlider(instance.GetRect(22f), value, min, max, false, null, null, null, -1f);
            if (!disabled && num != valueBefore)
            {
